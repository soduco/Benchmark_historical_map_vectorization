...
def getTopoLoss(likelihood_tensor, gt_tensor, topo_size=100, loss_type='MSE'):
    """
    ...
    if loss_type == 'BCE':
        topo_cp_ref_map = torch.tensor(topo_cp_ref_map).unsqueeze(0).unsqueeze(0).float().cuda()
        loss_topo = nn.BCEWithLogitsLoss(reduction='sum')(likelihood_tensor, topo_cp_ref_map)
    elif loss_type == 'MSE':
        topo_cp_ref_map = torch.tensor(topo_cp_ref_map, dtype=torch.float).cuda()
        loss_topo = (((likelihood_tensor * topo_cp_weight_map) - topo_cp_ref_map) ** 2).sum()
    # print('')
    # print('{} number of points reuqires to fix.'.format(len(torch.nonzero(topo_cp_ref_map))))
    return loss_topo
